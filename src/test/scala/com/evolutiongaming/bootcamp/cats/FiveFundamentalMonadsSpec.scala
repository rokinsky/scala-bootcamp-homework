...
import FiveFundamentalMonads._
import org.scalatest.matchers.should.Matchers
import org.scalatest.propspec.AnyPropSpec
import org.scalatestplus.scalacheck.ScalaCheckPropertyChecks

class FiveFundamentalMonadsSpec extends AnyPropSpec with ScalaCheckPropertyChecks with Matchers {
  def leftASSO[M[_], A, B, C](amb: A => M[B])(bmc: B => M[C])(ma: M[A])(implicit m: Monad[M]): M[C] =
    m.bind(m.bind(ma)(amb))(bmc)
  def rightASSO[M[_], A, B, C](amb: A => M[B])(bmc: B => M[C])(ma: M[A])(implicit m: Monad[M]): M[C] =
    m.bind(ma)((a: A) => m.bind(amb(a))(bmc))
  ...
  property("Maybe") {
    ...
    def isEven(x: Int): Maybe[Int] = if (x % 2 == 0) { Just(x) }
    else { None }
    ...
    forAll { (x: Int, y: Int, z: Int) =>
      {
        ...
        def f(a: Int): Maybe[Int] = if (x % 2 == 0) { isEven(a) }
        else { None }
        def g(a: Int): Maybe[Int] = if (y % 2 == 0) { isEven(a) }
        else { None }
  ...
  property("State") {
    def m = implicitly[Monad[State[Int, *]]]
  ...
  property("Reader") {
    def m = implicitly[Monad[Reader[Int, *]]]
  ...
  property("Writer") {
    ...
    def m = implicitly[Monad[Writer[Int, *]]]
