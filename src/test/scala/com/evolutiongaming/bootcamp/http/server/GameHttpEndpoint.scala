...
sealed abstract class GameHttpEndpoint[F[_]: Sync](guessService: GuessService[F]) extends Http4sDsl[F] {
  ...
  private[this] def createGame(request: Request[F]): F[Response[F]] =
    for {
  ...
  private[this] def tryGuess(request: Request[F], uuid: UUID): F[Response[F]] =
    for {
  ...
  private[this] def intercept: Throwable => F[Response[F]] = {
