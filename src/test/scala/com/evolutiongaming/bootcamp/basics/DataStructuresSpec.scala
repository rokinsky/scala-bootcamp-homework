...
class DataStructuresSpec extends AnyFlatSpec with ScalaCheckDrivenPropertyChecks {
  ...
  it should "work on 1 element list" in {
    allEqual(List("a")) shouldBe true
  }

  it should "work for lists which are NOT all equal" in {
    allEqual(List("a", "a", "b", "a")) shouldBe false
  }

  behavior of "totalVegetableCost"

  it should "be correct" in {
    totalVegetableCost shouldEqual 5012
  }

  behavior of "totalVegetableWeights"
  ...
  behavior of "allSubSetsOfSizeN"

  it should "work correctly on 2 from Set(1, 2, 3)" in {
